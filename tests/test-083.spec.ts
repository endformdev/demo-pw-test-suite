 ...
 */
test("performance-billing-test-83", async ({ page }) => {
  // Test scenario 83
  await runComplexTestScenario(page, 83);
  ...
  await test.step("Verifying results", async () => {
    ...
    await expect(page.locator('#test-result')).toHaveText(`Test 83 completed successfully`);
  });
});
