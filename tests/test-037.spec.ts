...
test("accessibility-settings-test-37", async ({ page }) => {
  ...
  await test.step("Verifying results", async () => {
    ...
    await expect(page.locator('#test-result')).toHaveText(`Test 37 completed successfully`);
  });
});
