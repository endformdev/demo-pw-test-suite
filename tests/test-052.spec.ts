...
/**
 * Complex ui test for analytics - variation 1
 * This test is designed to take 20-90 seconds to execute
 */
test("ui-analytics-test-52", async ({ page }) => {
  // Test scenario 52
  await runComplexTestScenario(page, 52);
  ...
  await test.step("Verifying results", async () => {
    ...
    await expect(page.locator('#test-result')).toHaveText(`Test 52 completed successfully`);
  });
});
