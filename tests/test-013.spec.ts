...
/**
 * Complex performance test for authentication - variation 1
 * This test is designed to take 20-90 seconds to execute
 */
test("performance-authentication-test-13", async ({ page }) => {
  // Test scenario 13
  await runComplexTestScenario(page, 13);
  ...
  await test.step("Verifying results", async () => {
    ...
    await expect(page.locator('#test-result')).toHaveText(`Test 13 completed successfully`);
  });
});
