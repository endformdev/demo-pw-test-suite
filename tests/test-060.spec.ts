...
/**
 * Complex e2e test for analytics - variation 1
 * This test is designed to take 20-90 seconds to execute
 */
test("e2e-analytics-test-60", async ({ page }) => {
  // Test scenario 60
  await runComplexTestScenario(page, 60);
  ...
  await test.step("Verifying results", async () => {
    ...
    await expect(page.locator('#test-result')).toHaveText(`Test 60 completed successfully`);
  });
});
